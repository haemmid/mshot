...
import { createWriteStream, statSync, mkdirSync, renameSync, unlinkSync, readFileSync } from "node:fs";
...
if (!url.startsWith("http://") && !url.startsWith("https://")) {
  fatal(`unsupported url protocol "${url.split(":")[0]}"; use http:// or https://`);
}
...
if (!ALLOWED_EXTS.includes(ext)) {
  fatal(
    `unsupported output extension "${ext}"; use .jpg, .jpeg, .png, or .webp`
  );
}
...
const format = ext === ".webp" ? "webp" : (ext === ".png" ? "png" : "jpeg");
...
try {
  ...
  tmpFile = outFile + ".tmp-" + randomUUID();
  ...
  tmpFile = null; // renamed, no cleanup needed
