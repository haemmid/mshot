...
export function parseArgsConfig() {
  return {
    options: {
      ...
      'segment-height': { type: 'string' },
      'segment-overlap': { type: 'string' },
...
Single-mode Options:
  ...
  --settle-timeout <ms>   Settle timeout ceiling (default ${DEFAULTS.settleTimeout})
...
Batch Options:
  ...
  --max-pages <n>         Max pages to capture, default 12
  ...
  Segmented output (batch only):
  --segments              Create overview + overlapping segments per viewport
  --segment-height <px>   Segment height (default ${SEGMENT_DEFAULTS.height})
  --segment-overlap <px>  Overlap between segments (default ${SEGMENT_DEFAULTS.overlap})
...
export function validateSingle(values) {
  ...
  return {
    ...
    isSettleEnabled: !values['no-settle'],
    settleTimeout: values['settle-timeout']
      ? parseInt(values['settle-timeout'], 10)
      : DEFAULTS.settleTimeout,
    networkidleTimeout: values['networkidle-timeout']
...
export function validateBatch(values) {
  ...
  const routeDedupe = !values['no-route-dedupe']
  ...
  // Segments validation
  const segmentsEnabled = values.segments === true
  let segmentHeight = SEGMENT_DEFAULTS.height
  let segmentOverlap = SEGMENT_DEFAULTS.overlap

  if (values['segment-height'] !== undefined) {
    segmentHeight = parseInt(values['segment-height'], 10)
    if (!Number.isSafeInteger(segmentHeight) || segmentHeight <= 0) {
      return {
        error: `--segment-height must be a positive integer, got ${values['segment-height']}`
      }
    }
  }

  if (values['segment-overlap'] !== undefined) {
    segmentOverlap = parseInt(values['segment-overlap'], 10)
    if (!Number.isSafeInteger(segmentOverlap) || segmentOverlap < 0) {
      return {
        error: `--segment-overlap must be a non-negative integer, got ${values['segment-overlap']}`
      }
    }
    if (segmentOverlap >= segmentHeight) {
      return {
        error: `--segment-overlap (${segmentOverlap}) must be less than --segment-height (${segmentHeight})`
      }
    }
  }

  // segment-height/overlap only valid with --segments
  if (
    !segmentsEnabled &&
    (values['segment-height'] !== undefined ||
      values['segment-overlap'] !== undefined)
  ) {
    return {
      error: '--segment-height and --segment-overlap require --segments'
    }
  }
