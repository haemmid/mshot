...
//   parseViewports, validateSingle, validateBatch
// Internal helpers:
//   parseNumeric, clampPositive
...
// ── parseNumeric(raw, fallback) → number ──────────────────
export function parseNumeric(raw, fallback) {
  if (raw == null || raw === '') return fallback
  const n = parseInt(raw, 10)
  return Number.isNaN(n) ? fallback : n
}

// ── clampPositive(value, min, max) → number ──────────────
export function clampPositive(value, min, max) {
  if (value < min) return min
  if (max !== undefined && value > max) return max
  return value
}

// ── parseViewports (re-export for convenience) ────────────
// Already defined above — kept as named export.
